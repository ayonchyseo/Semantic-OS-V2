...
export async function callGemini(prompt: string, options: GeminiOptions = {}): Promise<any> {
  ...
  try {
    ...
    // Safely extract JSON whether it's wrapped in markdown or not
    ...
    if (options.useGrounding) {
      if (jsonMatch) {
        try {
          ...
          return { raw: text, error: "Extracted grounding JSON was invalid" };
        }
      }
    ...
    try {
      // Try parsing the extracted block first, as it removes conversational padding
      if (jsonMatch) {
        return JSON.parse(jsonMatch[1] || jsonMatch[0]);
      }
      // If no structural match, try direct parsing (might fail, but let the catch handle it)
      return JSON.parse(text);
    } catch (parseError) {
      console.error("Gemini JSON Parse Error. Raw Text:", text);
      throw parseError; // Let the catch block handle the message format
    }
